...
public class ContoCorrente {
	...
	public void preleva(double importo) {
		try {
			Thread.sleep(4000); /* metto il thread in attesa per simulare l`accesso al database per il prelievo */
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		System.out.println("Prelievo " + importo + " dal conto " + getId());
	...
	public void versa(double importo) {
		...
		System.out.println("Verso " + importo + " dal conto " + getId());
		setSaldo(getSaldo() + importo);
	}
	...
	public static void trasferisci(ContoCorrente da, ContoCorrente a, double importo) {
		...
		synchronized(da) {
			...
			synchronized(a) {
				System.out.println("Avvio il versamento da " + a.id);
				a.versa(importo);
			}
		}
	}
